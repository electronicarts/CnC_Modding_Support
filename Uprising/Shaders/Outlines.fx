...
static const float4 OutlineSelectedColor = float4(1.0f, 0.713f, 0.0f, 1.0f);
...
static const float  OutlineIntensity = 5.0f;
...
VS_OUTPUT_BLUR_BOX VS_Blur_Box(float3 Position : POSITION, float3 TexCoord : TEXCOORD0)
{
	...
	OUT.NegX_PosX_NegY_PosY.x = TexelIncrement.x * -TEXEL_DISTANCE_FROM_CENTER;
	OUT.NegX_PosX_NegY_PosY.y = TexelIncrement.x *  TEXEL_DISTANCE_FROM_CENTER;
	OUT.NegX_PosX_NegY_PosY.z = TexelIncrement.y * -TEXEL_DISTANCE_FROM_CENTER;
	OUT.NegX_PosX_NegY_PosY.w = TexelIncrement.y *  TEXEL_DISTANCE_FROM_CENTER;

	OUT.Coord = TexCoord.xy;
	return OUT;
}

float4 PS_Blur_Box(VS_OUTPUT_BLUR_BOX IN) : COLOR
{
	float4 color = tex2D(SAMPLER(PostEffectOutlineTexture), IN.Coord + IN.NegX_PosX_NegY_PosY.xz);
	color += tex2D(SAMPLER(PostEffectOutlineTexture), IN.Coord + IN.NegX_PosX_NegY_PosY.xw);
	color += tex2D(SAMPLER(PostEffectOutlineTexture), IN.Coord + IN.NegX_PosX_NegY_PosY.yz);
	color += tex2D(SAMPLER(PostEffectOutlineTexture), IN.Coord + IN.NegX_PosX_NegY_PosY.yw);

	return OutlineIntensity * (color/4);
}
...
{
	return tex2D( SAMPLER(PostEffectOutlineTexture), TexCoord );
}
