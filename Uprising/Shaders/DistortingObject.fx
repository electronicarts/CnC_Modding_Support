...
<
	string UIName = "Distortion Strength";
	string UIWidget = "Slider";
	float UIMin = 0.0f;
	float UIMax = 1.0f;
	float UIStep = .01f;
...
bool CullingEnable
<
	string UIName = "Backface Cull Enable";
> = false;

bool ZWriteEnable
<
	string UIName = "Write to Z Buffer Enable";
> = false;
			...
			uniform int numJointsPerVertex)
{
	...
	Out.Alpha = VertexColor.w * OpacityOverride * DistortionStrength;
...
float4 PS(VSOutput In) : COLOR
{
	...
	float3 bumpNormal = normalMapSample.xyz * 2.0 - 1.0; // Normalized

	bumpNormal.xy *= BumpScale; // Scale normal to increase/decrease bump effect
	bumpNormal = normalize(bumpNormal); // Normalized
	
	float3 normal = (mul(bumpNormal, In.TangentToViewSpace)) * .5 + .5; // Get geometery normal and normalize
	float4 color = float4(normal, In.Alpha * normalMapSample.w);

#if !defined(_3DSMAX_)	
	color.w *= tex2D( SAMPLER(ShroudTexture), In.ShroudTexCoord).w;
#endif

//	return float4(In.Color.xyz * normal, In.Color.w * normalMapSample.w);
