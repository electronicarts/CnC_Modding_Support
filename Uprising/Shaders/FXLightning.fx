...
bool UV2Enable
<
	string UIName = "Use UV Channel 2 for Mask";
> = false;
...
#if defined(SUPPORT_PROTONCOLLIDER)
	Out.MaskTexCoord.xy = TexCoord1.xy;
	Out.MaskTexCoord.z = (OpacityOverride * 3)- 1;
#endif	
...
float4 PS(VSOutput In) : COLOR
{
	...
	float3 maskDiffuse = tex2D( SAMPLER(Texture_2), (In.DiffuseTexCoord.xy - In.MaskTexCoord.z));
	
	if (UV2Enable)
	{
		maskDiffuse = tex2D( SAMPLER(Texture_2), (In.MaskTexCoord.xy - In.MaskTexCoord.z));
	}
