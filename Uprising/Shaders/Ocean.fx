...
struct VSOutputHigh
{
    ...
    float4 ShroudTexCoord_FoamScaleOffset	: TEXCOORD5;
};
...
// -------------------------------------------------------------------------------
	...
	Out.ShroudTexCoord_FoamScaleOffset.xy = CalculateShroudTexCoord(Shroud, worldPosition);

	float2 foamScaleOffset = float2(300 / FoamBlend, -FoamHeight / FoamBlend - 1);
#if defined(EA_PLATFORM_WINDOWS)
	// On Windows do the actual foam strength calculation in the pixel shader as it will also work on non-vertex texturing hardware
	Out.ShroudTexCoord_FoamScaleOffset.zw = foamScaleOffset;
#else
	// On console compute use the wave displacement we already have to calculate the foam strength directly
	float foamStrength = waveDisplacement.z * foamScaleOffset.x + foamScaleOffset.y;
	Out.ShroudTexCoord_FoamScaleOffset.zw = foamStrength;
#endif
...
float4 PS_ULTRAHIGH(VSOutputHigh In, uniform bool hasShadow) COLORTARGET
{
...
#if defined(EA_PLATFORM_WINDOWS)
	// On Windows do the actual foam strength calculation in the pixel shader as it will also work on non-vertex texturing hardware
	float2 foamScaleOffset = In.ShroudTexCoord_FoamScaleOffset.zw;
	float HeightBlendFactor = heightCenter * foamScaleOffset.x + foamScaleOffset.y;
#else
	// On console use the foam strength from the VS
	float HeightBlendFactor = In.ShroudTexCoord_FoamScaleOffset.z;
#endif

	FoamSamp = pow(FoamSamp,2) * clamp(HeightBlendFactor * .2,.025,3) * DirectionalLight[0].Color;
	...
	float shroud = tex2D( SAMPLER(ShroudSampler), In.ShroudTexCoord_FoamScaleOffset.xy).x;
...
VSOutputMedium VS_MEDIUM(float2 PositionXY : POSITION)
{
	...
	worldPosition.z -= 2; // There are too many areas where only on medium the water shows up through very shallow terrain areas. To not have to rework the terrain, we lower the water globally a bit.
...
float4 PS_LOW(VSOutputLow In) : COLOR
{
	...
	// Apply shroud
	color.xyz *= tex2D( SAMPLER(ShroudSampler), In.ShroudTexCoord_Foam.xy).x;
