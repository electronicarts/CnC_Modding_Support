...
static const int BLEND_MODE_MOVE_MARKER_LINE_DRAW = 7;
static const int NUM_BLEND_MODES = 8;
...
VSOutput VS_MarkerLine(float3 Position : POSITION, float2 TexCoord0 : TEXCOORD0, float4 VertexColor : COLOR0)
{
	VSOutput Out;
        float2 fMoveRate = float2(0.0f,-0.5f);
	...
	Out.BaseTexCoord = (fMoveRate * Time) + TexCoord0;
	
	return Out;
}

float4 PS_MarkerLine(VSOutput In) : COLOR
{
	// Get vertex color
	float4 color = In.DiffuseColor;
...
technique MarkerLineDraw
{
	pass P0
	{
		VertexShader = compile VS_VERSION_LOW VS_MarkerLine();
		PixelShader = compile PS_VERSION_LOW PS_MarkerLine();
