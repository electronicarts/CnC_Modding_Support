...
float4 PS(VSOutput In, uniform bool applyGammaCorrection) COLORTARGET
{
	// Get vertex color
...
float4 PSLightning(VSLightningOutput In, uniform bool applyGammaCorrection) COLORTARGET
{
	// Get vertex color
	float4 color = In.DiffuseColor;
	...
	float2 textureDisplace = tex2D(SAMPLER(Texture1), In.DistortionTexCoord.xy).xy;
	textureDisplace += tex2D(SAMPLER(Texture1), In.DistortionTexCoord.zw).xy;
...
float4 PSLightning_GammaCorrection( VSLightningOutput In ) : COLOR
{
	return PSLightning(In, true);
}

float4 PSLightning_NoGammaCorrection( VSLightningOutput In ) : COLOR
{
	return PSLightning(In, false);
}
...
technique AdditiveAlphaTest
{
	pass P0
	{
		...
		PixelShader = compile PS_2_0 PSLightning_GammaCorrection();
...
technique AdditiveAlphaTest_M
{
	...
	{
		VertexShader = compile VS_2_0 VSLightning();
		PixelShader = compile PS_2_0 PSLightning_NoGammaCorrection();
