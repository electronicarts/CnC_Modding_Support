             ...
             float2 NormalTexCoord : TEXCOORD1 )
{
    ...
    // Fog
	float3 worldPosition = mul( float4(Position, 1), World ).xyz;
...
float4 PS( VSOutput In ) : COLOR
{
	...
	float2 normalMapSample1 = tex2D( SAMPLER(NormalTexture), In.NormalTexCoord.xy ).xy * 2 - 1;
	float2 normalMapSample2 = tex2D( SAMPLER(NormalTexture), In.NormalTexCoord.wz ).xy * 2 - 1;
	...
	float3 reflectionColor = tex2D( SAMPLER(WaterReflectionTexture), reflectionCoord ).xyz;
...
float4 PS_M( VSOutput_M In ) : COLOR
{
	...
	float3 reflectionColor = texCUBE(SAMPLER(EnvironmentTexture), reflectionDirection).xyz;
	color.xyz += reflectionColor * 0.75;
