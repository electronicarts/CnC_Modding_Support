...
#if !defined(FORCE_PARTICLES_INTO_XY_PLANE) // TODO: Implement particle processor for XY-plane aligned particles
...
#if defined(DRAW_UNDERWATER)
#include "CommonUnderwater.fxh"
#endif // defined(DRAW_UNDERWATER)
...
#if defined(DRAW_UNDERWATER)
	Out.WorldPosition = cornerPosition;
#endif
...
#if defined(DRAW_UNDERWATER)
	// Apply underwater tinting
	Color.xyz *= GetDepthLutValue(In.WorldPosition).xyz;
#endif
...
technique Default
{
	...
	>
	{
		...
		PixelShader = ARRAY_EXPRESSION_PS( PS_Array_H,
			(!ShouldDrawParticleSoft ? SOFTBLEND_DISABLED : ((Draw_ShaderType == ShaderType_Alpha) ? SOFTBLEND_ALPHA : SOFTBLEND_ADDITIVE)) * PS_Multiplier_ShaderType
#if SUPPORT_FOG
...
#if defined(DRAW_UNDERWATER)
	// Apply underwater tinting
	Color.xyz *= GetDepthLutValue_M(In.WorldPosition).xyz;
#endif
