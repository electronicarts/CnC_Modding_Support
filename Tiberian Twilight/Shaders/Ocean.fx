...
// -------------------------------------------------------------------------------
...
#if defined(EA_PLATFORM_PS3)
	// $note (WSK) 2008/10/24 - On ps3, we bind the displacement texture as stream 1 since texture fetching on vertex shader is super expensive on ps3.
	//							Using yzw because of the wierd color swizzle for A8R8G8B8 in RNA
	float3 waveDisplacement = Displacement.yzw;
	...
	worldPosition += waveDisplacement * float3(-300.0, 300.0, 300.0);
#else
	float waveValue = 0.0;
...
float4 PS_ULTRAHIGH(VSOutputHigh In, uniform bool hasShadow, float2 vPos : PIXELLOC) COLORTARGET
{
	...
	float3 FoamSamp = tex2D(SAMPLER(Foam), FoamTexCoords.xy).xyz *
					  tex2D(SAMPLER(Foam), FoamTexCoords.zw).xyz;
...
#if defined(DO_DISPLACE_NORMAL)
	float2 PositionXY = In.ShroudTexCoord_Displacement.zw;
	float heightCenter = tex2D(SAMPLER(DisplacementTexture), PositionXY).z;
	float heightX = tex2D(SAMPLER(DisplacementTexture), PositionXY + float2(OneOverDisplacementTextureSize, 0)).z;
	float heightY = tex2D(SAMPLER(DisplacementTexture), PositionXY + float2(0, OneOverDisplacementTextureSize)).z;

	float3 nDisp = float3(heightX - heightCenter, heightY - heightCenter, 0.01) * 50;
	float3 Nn = normalize(nDisp + staticDisplacement * .2);
#else // #if defined(DO_DISPLACE_NORMAL)
...
//			    || Refraction * Inverse fresnel       || Reflection
	...
	float3 waterColor = (colorDelta < maxColorDeltaToUseRefraction)?refractionColor:baseColor;
	float4 color = float4(waterColor + shroud * ((reflectionColor * .5) + FoamSamp + envcolor), 1.0);
...
float4 PS_MEDIUM(VSOutputMedium In) : COLOR
{
	...
	color.xyz = shroud * ((waterColor * float3(.16, .57, .79)) + envcolor * DirectionalLight_0_Color.xyz);
...
float4 PS_LOW(VSOutputLow In) : COLOR
{
	...
	float4 color = float4(In.Color.xyz + (envcolor * DirectionalLight_0_Color.xyz), In.Color.w);
