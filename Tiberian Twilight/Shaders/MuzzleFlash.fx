...
//	VertexColor.w = 2.4; //Debug for MAX as it does not read Vertex Color
	...
	Out.DiffuseColor = VertexColor.xyz * ColorEmissive;
...
// ----------------------------------------------------------------------------
float4 PS(VSOutput In, uniform int blendMode, uniform bool applyGammaCorrection)
{
	// Get vertex color
	...
	float3 shroud = tex2D(SAMPLER(ShroudTexture), In.ShroudTexCoord).xxx;
...
// ----------------------------------------------------------------------------
float4 PS_Additive_WithGammaCorrection(VSOutput In) : COLOR0
{
	return PS(In, BlendMode_Additive, true);
}

float4 PS_Additive_WithNoGammaCorrection(VSOutput In) : COLOR0
{
	return PS(In, BlendMode_Additive, false);
}

float4 PS_Multiplicative_WithGammaCorrection(VSOutput In) : COLOR0
{
	return PS(In, BlendMode_Multiplicative, true);
}

float4 PS_Multiplicative_WithNoGammaCorrection(VSOutput In) : COLOR0
{
	return PS(In, BlendMode_Multiplicative, false);
}
...
technique Additive
{
	pass P0
	{
		...
		PixelShader = compile PS_2_0 PS_Additive_WithGammaCorrection();
...
technique Multiply
{
	pass P0
	{
		...
		PixelShader = compile PS_2_0 PS_Multiplicative_WithGammaCorrection();
...
technique Additive_M
{
	pass P0
	{
		...
		PixelShader = compile PS_2_0 PS_Additive_WithNoGammaCorrection();
...
technique Multiply_M
{
	pass P0
	{
		...
		PixelShader = compile PS_2_0 PS_Multiplicative_WithNoGammaCorrection();
