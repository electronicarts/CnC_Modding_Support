...
// Environment Texture
// ----------------------------------------------------------------------------
// $note (WSK) 2008/10/15 - the environment texture seems to be unused in pixel shader, so I comment out everything related to Environment texture
/*
...
VSOutput VS( VSInput Input )
{
    ...
    float3 velocity_vec_x = normalize( cross( velocity_vec_y, view_pos_center.xyz ) );
    ...
    Output.LightColor_Fog.xyz = float3((DirectionalLight_0_Color.xyz + DirectionalLight_1_Color.xyz + DirectionalLight_2_Color.xyz) * .7);
...
{
    // Get the base color
    float4 diffuseColor = float4(.45,.9,5,1);
    float4 diffuse_texture = tex2D( SAMPLER(DiffuseTexture), Input.DiffuseTexCoord );
    float4 Color = diffuse_texture * Input.Color * diffuseColor * 2;
