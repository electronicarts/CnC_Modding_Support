...
// ----------------------------------------------------------------------------
float3 GetCausticsColor(float3 worldPosition, float3 worldNormal, float4 directionalLight_color, float4 directionalLight_direction, float time, uniform bool isFullColor)
{
	// Generate Caustic Tex Coords Based on Sunlight Vector
	float2 CausticTexCoord = (worldPosition.xy - worldPosition.z * directionalLight_direction.xy / directionalLight_direction.z) * .004;
	...
	float2 causticDistort = tex2D( SAMPLER(CausticSampler), causticTexCoordsDiverge).xy;
	causticDistort += tex2D( SAMPLER(CausticSampler), causticTexCoordsDivergeInv).xy;
	...
	float mainLightFalloff = max(0, dot(worldNormal, directionalLight_direction.xyz));
	
	float3 caustics = causticSamp * depthLut.w * lerp(directionalLight_color.rgb,float3(1,1,1),.75) * mainLightFalloff;
