...
#define SQUARED_BLOOM_CURVE
	...
	float diff = max(intensity-threshold, 0.0);
	return diff*diff;
...
// ----------------------------------------------------------------------------
float4 DownsampleInitialPS(VSOutput In, uniform int halfKernelSize) COLORTARGET
{
	...
	// $todo (WSK) 2009/01/09 - Unfortunately, we couldn't find 100% of the case where the shader could write invalid or infinity in the framebuffer
	//                          Most of those cases are when shader write a > 1.0 alpha values, or normalize a value that is 0 lenght, or divide by 0 somewhere.
	//                          This is a safe guard here to avoid having bloom create a big black quad on screen.
	//                          If you want to debug the case where shader output bad values, make the color bright pink, like float4(32.0, 0.0, 32.0, 1.0).
	if(dot(isinf(color), 1.0) > 0.0 || dot(isnan(color), 1.0) > 0.0)
	{
		// Set to black to avoid spreading an invalid values
		color = float4(0.0, 0.0, 0.0, 1.0);
	}

	// Clamp the color so we don't have bogus values
	color = min(color, 32.0.xxxx);
...
// ----------------------------------------------------------------------------
float4 DownsampleInitialPS_KernalSize_2(VSOutput In) : COLOR
{
	return DownsampleInitialPS(In, 2);
}

// ----------------------------------------------------------------------------
float4 BlurGaussian11x11PS(VSOutput In, uniform float2 direction)
{
...
// ----------------------------------------------------------------------------
float4 BlurGaussian11x11PSU(VSOutput In) : COLOR
{
	return BlurGaussian11x11PS(In, float2(1, 0));
}

// ----------------------------------------------------------------------------
float4 BlurGaussian11x11PSV(VSOutput In) : COLOR
{
	return BlurGaussian11x11PS(In, float2(0, 1));
}
...
// ----------------------------------------------------------------------------
float4 AccumulateFinalAdditivePS(float2 TexCoord : TEXCOORD0) : COLOR
{
...
technique DownsampleInitial
{
	pass p0
	{
		...
		PixelShader = ARRAY_EXPRESSION_PS(DownsampleInitialPS_Array,
			...
			compile PS_3_0 DownsampleInitialPS_KernalSize_2()
		);
...
technique DownsampleInitial_L
{
	pass p0
	{
		...
		PixelShader = ARRAY_EXPRESSION_PS(DownsampleInitialPS_L_Array,
			...
			// Non-array alternative:
			// Xenon always has a fixed screen size, so the downsample ratio can be hardcoded here to be 4x4 (= half kernel size 2)
			compile PS_2_0 DownsampleInitialPS_KernalSize_2()
		);
...
technique BlurGaussian11x11U
{
	...
	{
		VertexShader = compile VS_2_0 DefaultVS();
		PixelShader = compile PS_2_0 BlurGaussian11x11PSU();
...
technique BlurGaussian11x11V
{
	...
	{
		VertexShader = compile VS_2_0 DefaultVS();
		PixelShader = compile PS_2_0 BlurGaussian11x11PSV();
...
// ----------------------------------------------------------------------------
technique AccumulateFinalAdditive
{
	pass p0
	{
		VertexShader = compile VS_3_0 DefaultVS();
		PixelShader = compile PS_3_0 AccumulateFinalAdditivePS();
